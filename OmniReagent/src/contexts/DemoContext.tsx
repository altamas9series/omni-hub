...
export interface Document {
  ...
  status: "pending" | "processing" | "processed" | "exception" | "duplicate" | "irrelevant";
  confidence: number;
  uploadedAt: Date;
  processedAt?: Date;
  size: number;
  source: "upload" | "email" | "drive" | "onedrive" | "sharepoint" | "dropbox" | "legacy_archive" | "cloud_storage";
  extractedData?: Record<string, string>;
  thumbnail?: string;
  apiId?: number; // Store original API ID
}
...
const sampleDocuments: Document[] = [
  ...
  },
  {
    ...
    source: "drive",
    extractedData: {
      "Parties": "TechCorp Inc. & OmniReagent",
...
export function DemoProvider({ children }: { children: ReactNode }) {
  ...
  const refreshDocuments = async () => {
    // In a real generic context, this might cause circular dependency if we import service here
    // But for this specific task, we want the context to manage the source of truth.
    // However, DemoContext seems to be a mock context.
    // Ideally we should move to a real FileContext, but the user asked to "Add API", so I will patch DemoContext or just fetch in the view and setDocuments?
    // Let's modify DemoContext to be "hybrid" - initially mock but can load from API.
    try {
      const { fileService, FileStatus, FileSource } = await import("@/services/files");
      const response = await fileService.getFiles(1, 100); // Get first 100 for now

      const mappedDocs: Document[] = response.items.map(apiFile => {
        let type: Document['type'] = "form"; // Default
        if (apiFile.filename.toLowerCase().includes("invoice")) type = "invoice";
        else if (apiFile.filename.toLowerCase().includes("sheet")) type = "manual";

        let status: Document['status'] = "pending";
        if (apiFile.status === FileStatus.PROCESSING) status = "processing";
        else if (apiFile.status === FileStatus.INDEXED) status = "processed";
        else if (apiFile.status === FileStatus.VERIFIED) status = "processed";
        ...
        else if (apiFile.source === FileSource.EMAIL) source = "email";
        ...
        return {
          ...
          type,
          status,
          confidence: apiFile.chemicals_count > 0 ? 100 : 0, // Placeholder logic
      ...
      // Merge with existing samples or replace? 
      // Let's append real ones to samples for "Demo" feeling, or just replace?
      // User probably wants to see THEIR files. Let's replace.
